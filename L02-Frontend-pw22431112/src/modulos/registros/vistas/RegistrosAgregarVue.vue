...
<template>
    <div class="container-fluid d-flex justify-content-center align-items-center min-vh-100">
      <div class="card w-100 col-lg-8 col-md-10 col-sm-12 shadow-lg bg-light small-card">
        ...
        <div class="alert alert-success text-center mt-3" v-if="mensaje && mensaje[0] === 'Registro agregado con éxito'" role="alert">
          {{ mensaje[0] }}
        </div>
        <div class="alert alert-danger text-center mt-3" v-for="(error, index) in mensaje" :key="index" v-if="mensaje && mensaje[0] !== 'Registro agregado con éxito'" role="alert">
          {{ error }}
        </div>
...
.border-custom {
  border: 3px solid #5cb85c; /* Verde más oscuro */
  border-radius: 20px;
